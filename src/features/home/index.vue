...
const fetchSourcesList = async () => {
  const data = await requestJsonWithRetry(apiUrl('/api/sources'))
...
const fetchSourceItems = async (id, { latest = false } = {}) => {
  ...
  try {
    return await requestJsonWithRetry(apiUrl(`/api/search?${params.toString()}`))
  } catch (error) {
...
const buildBoards = async ({ runSilentRefresh = true } = {}) => {
  ...
  try {
    ...
    if (allSelectedSources.value.length) {
      ...
      if (runSilentRefresh) void silentRefreshLoadedBoards()
    }
...
const onGlobalRefresh = () => refreshAll()
