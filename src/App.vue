<template>
  <div class="app-shell">
    ...
    <header class="app-header">
      ...
      >
        <button class="user-trigger" type="button" aria-label="Open account menu" @click="userMenuOpen = !userMenuOpen">
        ...
        >
          ...
          <template v-else>
            <button v-if="loginEnabled" class="menu-link" type="button" @click="loginWithGithub">GitHub Login</button>
            <p v-else class="menu-hint">GitHub login not configured</p>
          </template>
        </div>
      </div>
    </header>
...
const isLoggedIn = computed(() => Boolean(userStore.authToken))
...
const syncProfile = async () => {
  ...
  try {
    ...
    userStore.setAuth({
      ...
      type: userStore.loginType || profile.user.type || 'github',
      profile: {
        ...(userStore.profile || {}),
        name: userStore.profile?.name || profile.user.login || profile.user.email || profile.user.id,
        login: profile.user.login || userStore.profile?.login || '',
        isAdmin: Boolean(profile.user.isAdmin),
        isBlacklisted: Boolean(profile.user.isBlacklisted)
      }
    })
  } catch {
